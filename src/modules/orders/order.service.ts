...
export class OrderService {
   ...
   */
  async createOrder(dto: CreateOrderDTO): Promise<{ order: IOrder; isDuplicate: boolean }> {
    ...
    if (idempotencyKey) {
      ...
      if (cached) {
        ...
        if (parsed.status === 'COMPLETED') {
          logger.info({ idempotencyKey }, 'Returning idempotent cached order response');
          return { order: parsed.order as IOrder, isDuplicate: true };
        }
      }

      // Mark as PROCESSING with 60s lock
      await redis.set(idempKey, JSON.stringify({ status: 'PROCESSING' }), 'EX', 60);
    }
    ...
    try {
      ...
      for (const item of dto.items) {
        if (item.reservationToken) {
          const committed = await inventoryService.commitReservation(item.reservationToken);
          if (!committed) {
            logger.warn({ item }, 'Could not commit existing reservation token, proceeding with order item');
          }
        }
      }
      ...
      if (idempotencyKey) {
        await redis.del(`idemp:${tenantId}:${idempotencyKey}`);
      }
      throw error;
    }
  }
  ...
  async transitionStatus(
    ...
    metadata?: { trackingNumber?: string; courierName?: string }
  ): Promise<IOrder> {
    ...
    if (!allowed.includes(newStatus)) {
      throw new Error(
        `Invalid status transition: Cannot transition order from ${currentStatus} to ${newStatus}. Allowed transitions: ${allowed.join(
          ', '
        )}`
      );
    }
    ...
    if (newStatus === 'SHIPPED') {
      ...
      if (metadata?.courierName) order.courierName = metadata.courierName;
    }

    order.status = newStatus;
    await order.save();

    logger.info({ tenantId, orderId, from: currentStatus, to: newStatus }, 'Order status transitioned');
