...
export async function seedData() {
  const existingTenants = await TenantModel.countDocuments();
  ...
  logger.info('🌱 Auto-seeding initial database records for Amader Bazar...');

  const passwordHash = await bcrypt.hash('Password123!', 10);

  // 1. Create Tenant (Amader Bazar)
  const tenantAmaderBazar = await TenantModel.create({
  ...
  logger.info('✅ Amader Bazar database records seeded successfully');
}

async function seedSlidersAndReviews(tenantId: string) {
  // Create Sliders
  await SliderModel.deleteMany({ tenantId });
  ...
  const honey = await ProductModel.findOne({ tenantId, slug: 'sundarban-raw-natural-honey' });
  if (honey) {
    await ReviewModel.deleteMany({ tenantId, productId: honey._id });
