...
class OrderController {
    async createOrder(req, res) {
        try {
            ...
            const { order, isDuplicate } = await order_service_1.orderService.createOrder({
                tenantId,
    ...
    async getOrderById(req, res) {
        try {
            ...
            const order = await Order_1.OrderModel.findOne(query).lean();
            if (!order)
    ...
    async transitionStatus(req, res) {
        try {
            ...
            const updated = await order_service_1.orderService.transitionStatus(tenantId, id, status, {
                trackingNumber,
                courierName,
            });
            res.json({
