...
async function seedData() {
    const existingTenants = await Tenant_1.TenantModel.countDocuments();
    if (existingTenants > 0) {
        // Check if sliders or reviews need seeding
        const sliderCount = await Slider_1.SliderModel.countDocuments();
        if (sliderCount === 0) {
            await seedSlidersAndReviews('tenant-fashion-001');
        }
        return;
    }
    logger_1.logger.info('🌱 Auto-seeding initial database records for Amader Bazar...');
    ...
    const tenantAmaderBazar = await Tenant_1.TenantModel.create({
        ...
        settings: {
            currency: 'BDT',
            primaryColor: '#16A34A', // Emerald / Forest Green
    ...
    // 4. Seed Sliders & Reviews
    await seedSlidersAndReviews(tenantAmaderBazar.tenantId);
    // 5. Seed Sample Orders with BDT and Bangladeshi logistics
    ...
    logger_1.logger.info('✅ Amader Bazar database records seeded successfully');
}
async function seedSlidersAndReviews(tenantId) {
    // Create Sliders
    await Slider_1.SliderModel.deleteMany({ tenantId });
    ...
    // Seed sample reviews for the first product
    const honey = await Product_1.ProductModel.findOne({ tenantId, slug: 'sundarban-raw-natural-honey' });
    if (honey) {
        await Review_1.ReviewModel.deleteMany({ tenantId, productId: honey._id });
